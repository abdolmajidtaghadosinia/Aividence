...
## ویژگی‌های محصول
- **نمای یکپارچه داشبورد**: کارت وضعیت پردازش به‌همراه نمودار پیشرفت، شمارش فایل‌های هر وضعیت و جدول فایل‌ها با برچسب وضعیت (Queued، Processing، Complete، Rejected) در همان صفحه ارائه می‌شود.
- **کنترل کامل خروجی**: برای هر فایل می‌توان نسخه پردازش‌شده یا محتوای ویرایش‌شده‌ی اختصاصی را انتخاب و در قالب‌های DOCX/PDF یا بسته Zip دریافت کرد؛ فونت فارسی و چینش راست‌به‌چپ برای خوانایی بهتر اعمال می‌شود.
- **جست‌وجوی لحظه‌ای**: فیلد جست‌وجوی بالای جدول در لحظه نام فایل‌ها را فیلتر می‌کند و نیازی به رفرش یا کوئری مجدد نیست.
- **ویرایش درون‌صفحه‌ای**: محتوای ترنسکرایب‌شده مستقیماً در صفحه بازبینی قابل ویرایش است و نسخه نهایی در همان رکورد ذخیره می‌شود.
- **آگاه‌سازی بلادرنگ**: تغییر وضعیت پردازش (شروع، ادامه، پایان یا خطا) با نوتیفیکیشن مرورگر، نوار پیشرفت و آیکون‌های وضعیت به‌روز می‌شود تا کاربر هیچ مرحله‌ای را از دست ندهد.
- **مدیریت واژه‌های کلیدی**: فهرست واژه‌های تخصصی در بخش واژه‌نامه نگهداری می‌شود تا موتور ترنسکرایب در حوزه‌های خاص عملکرد دقیق‌تری داشته باشد.
- **امنیت و هویت‌سنجی**: تمام API‌ها با JWT محافظت می‌شوند و دانلود خروجی فقط برای کاربران احراز هویت‌شده امکان‌پذیر است.
