...
```
نکته فونت PDF: برای نمایش صحیح فارسی در خروجی PDF، یک فونت راست‌به‌چپ روی سیستم نصب کنید (مثلاً `apt-get install fonts-vazirmatn` یا `fonts-dejavu`). به‌دلیل اجتناب از اضافه‌کردن فایل باینری به مخزن، فونت به‌صورت سیستمی فراهم می‌شود و برنامه در صورت وجود به‌طور خودکار آن را برای گزارش‌ها استفاده می‌کند.
